//=============================================================================
// Gameover Engine Plugins - Template
// YEP_GabWindow.js
//=============================================================================

var Imported = Imported || {};
Imported.GO_GabWindowNat = true;

var Gameover = Gameover || {};
Gameover.Gab = Gameover.Gab || {};
Gameover.Gab.version = 1.07

//=============================================================================
 /*:
 * @plugindesc v1.06 The Gab Window functions as a window for random
 * jibber jabber that does not require a message window.
 * @author Gameover Engine Plugins
 *
 * @param ---General---
 * @default
 *
 ...
 * ============================================================================
 *
 * This Code isn't from GameOver, check yanfly for more informations. The
 * code is for a little independant game.
 *
...
Gameover.Parameters = PluginManager.parameters('GO_GabWindowNat');
Gameover.Param = Gameover.Param || {};

Gameover.Param.GabFontName = String(Gameover.Parameters['Gab Font Name']);
Gameover.Param.GabFontSize = Number(Gameover.Parameters['Gab Font Size']);
Gameover.Param.GabCharPosX = Number(Gameover.Parameters['Character X Pos']);
Gameover.Param.GabCharPosY = Number(Gameover.Parameters['Character Y Pos']);
Gameover.Param.GabBaseTime = Number(Gameover.Parameters['Base Wait Time']);
Gameover.Param.GabTimePerChar = Number(Gameover.Parameters['Time Per Character']);
Gameover.Param.GabFadeRate = Number(Gameover.Parameters['Fade Rate']);
Gameover.Param.GabAntiRepeat = Boolean(Gameover.Parameters['Anti-Repeat']);

Gameover.Param.GabMapYLoc = Number(Gameover.Parameters['Map Y Location']);
Gameover.Param.GabMapDim1 = String(Gameover.Parameters['Map Dim Color 1']);
Gameover.Param.GabMapDim2 = String(Gameover.Parameters['Map Dim Color 2']);

Gameover.Param.GabBattleYLoc = Number(Gameover.Parameters['Battle Y Location']);
Gameover.Param.GabBattleDim1 = String(Gameover.Parameters['Battle Dim Color 1']);
Gameover.Param.GabBattleDim2 = String(Gameover.Parameters['Battle Dim Color 2']);

//=============================================================================
// Game_Interpreter
//=============================================================================

Gameover.Gab.Game_Interpreter_clear = Game_Interpreter.prototype.clear;
Game_Interpreter.prototype.clear = function() {
    Gameover.Gab.Game_Interpreter_clear.call(this);
    this.clearGabInformation();
};

Gameover.Gab.Game_Interpreter_pluginCommand =
    Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    Gameover.Gab.Game_Interpreter_pluginCommand.call(this, command, args)
    if (command === 'GabTextNat') this.setGabText(args);
    if (command === 'GabSpriteNameNat') this.setGabSpriteName(args);
    if (command === 'GabSpriteIndexNat') this.setGabSpriteIndex(args);
    if (command === 'GabFaceNameNat') this.setGabFaceName(args);
    if (command === 'GabFaceIndexNat') this.setGabFaceIndex(args);
    if (command === 'GabActorSpriteNat') this.setGabActorSprite(args);
    if (command === 'GabActorNat') this.setGabActorFace(args);
    if (command === 'GabActorFaceNat') this.setGabActorFace(args);
    if (command === 'GabPartySpriteNat') this.setGabPartySprite(args);
    if (command === 'GabPartyNat') this.setGabPartyFace(args);
    if (command === 'GabPartyFaceNat') this.setGabPartyFace(args);
    if (command === 'GabSoundNat') this.setGabSound(args);
    if (command === 'GabSwitchNat') this.setGabSwitch(args);
    if (command === 'ShowGabNat') this.showGab();
    if (command === 'ForceGabNat') this.forceGab();
    if (command === 'ClearGabNat') this.clearGab();
    if (command === 'WaitForGabNat') this.waitForGab();
};
...
Game_Interpreter.prototype.setGabText = function(args) { // modification Nathan
    var text = '';
...
Gameover.Gab.Game_Interpreter_updateWaitMode =
  Game_Interpreter.prototype.updateWaitMode;
Game_Interpreter.prototype.updateWaitMode = function() {
  if (this._waitMode === 'gab') {
    return this.isGabRunning();
  } else {
    return Gameover.Gab.Game_Interpreter_updateWaitMode.call(this);
  }
};
...
Window_Gab.prototype.initialize = function(battle) {
    ...
    var wx = Graphics.boxWidth/2 ; // modifier
    if (battle) {
      var wy = Gameover.Param.GabBattleYLoc - this.standardPadding();
    } else {
      var wy = Gameover.Param.GabMapYLoc - this.standardPadding() - 40; // modifier 40
    }
    var ww = Graphics.boxWidth + this.standardPadding() * 2;
    var wh = this.fittingHeight(3); // modifier 2
    this._gabSwitch = 0;
    this._showCount = 0;
...
Window_Gab.prototype.standardFontFace = function() {
    return Gameover.Param.GabFontName;
};

Window_Gab.prototype.standardFontSize = function() {
    return Gameover.Param.GabFontSize;
};
...
Window_Gab.prototype.updateFadeIn = function() {
    this.contentsOpacity += Gameover.Param.GabFadeRate;
};
...
Window_Gab.prototype.checkDuplicateGab = function(gabData) {
    if (!Gameover.Param.GabAntiRepeat) return false;
...
Gameover.Gab.WindowLayer_webglMaskWindow =
...
WindowLayer.prototype._webglMaskWindow = function(renderSession, win) {
    ...
    Gameover.Gab.WindowLayer_webglMaskWindow.call(this, renderSession, win);
};

Window_Gab.prototype.startCountdown = function() {
    this._graphicLoading = false;
    this.contentsOpacity = 255;
    this._showCount = Gameover.Param.GabBaseTime;
    var text = this._text.replace(/\\(.*?)\[(.*?)\]/gi, '');
    this._showCount += text.length * Gameover.Param.GabTimePerChar;
};

Window_Gab.prototype.drawGabBackground = function() {
    var width = this.contentsWidth();
    this.drawBackground(0, 0, width, this.lineHeight() * 4); // modifier
};

Window_Gab.prototype.dimColor1 = function() {
    if ($gameParty.inBattle()) {
      return Gameover.Param.GabBattleDim1;
    } else {
      return Gameover.Param.GabMapDim1;
    }
};

Window_Gab.prototype.dimColor2 = function() {
    if ($gameParty.inBattle()) {
      return Gameover.Param.GabBattleDim2;
    } else {
      return Gameover.Param.GabMapDim2;
    }
};
...
Window_Gab.prototype.drawGabCharacter = function() {
    ...
    if (this._graphicType === 'character') {
      var wx = Gameover.Param.GabCharPosX;
      var wy = Gameover.Param.GabCharPosY;
...
Window_Gab.prototype.drawGabText = function() {
    var text = this._text;
    if (this._graphicType === 'character') {
      var wx = Gameover.Param.GabCharPosX * 2;
    } else if (this._graphicType === 'face') {
...
Gameover.Gab.SceneManager_push = SceneManager.push;
SceneManager.push = function(sceneClass) {
    if (this.isStoreGabs(sceneClass)) {
      this._scene._gabWindow.storeGabs();
    }
    Gameover.Gab.SceneManager_push.call(this, sceneClass);
};
...
Gameover.Gab.Scene_Battle_createAllWindows =
    Scene_Battle.prototype.createAllWindows;
Scene_Battle.prototype.createAllWindows = function() {
    Gameover.Gab.Scene_Battle_createAllWindows.call(this);
...
Gameover.Gab.Scene_Map_createAllWindows = Scene_Map.prototype.createAllWindows;
Scene_Map.prototype.createAllWindows = function() {
    Gameover.Gab.Scene_Map_createAllWindows.call(this);
